...
class Experiment {
  ...
  next_block(restartGame, goBack) {
    ...
    this.block_count++;
    let block = new TrialBlock(
      this.block_index,
      this.block_count,
      this,
      this.block_configs[this.block_index],
    );
...
class TrialBlock {
  ...
  setTrialOrder() {
    ...
    let loc_list = this.makeLocationSequence(
      this.ncues,
      this.ntrials_per_cue,
      E.params.POSITIONS_PER_CUE,
    );
    ...
    this.cue_list = res;
    this.cue_locations = divideRange(
      riverX + cueWidth / 2,
      riverX + riverWidth - cueWidth / 2,
      E.params.POSITIONS_PER_CUE,
    );
  }
...
class Trial {
  ...
  trigger(event, callback) {
    // this is creating events in json
